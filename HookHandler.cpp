...
std::string HookHandler::getActiveProcessName()
{
    ...
    DWORD length = std::size(processName);
...
LRESULT CALLBACK HookHandler::onMouseEvent(int nCode, WPARAM wParam, LPARAM lParam)
{
    if (nCode == HC_ACTION)
    {
        ...
        if (!(mouse->flags & LLMHF_INJECTED))
        {
            ...
            switch (wParam)
            {
            case WM_LBUTTONDOWN: keyCode = VK_LBUTTON;
                down = true;
                break;
            case WM_RBUTTONDOWN: keyCode = VK_RBUTTON;
                down = true;
                break;
            case WM_MBUTTONDOWN: keyCode = VK_MBUTTON;
                down = true;
                break;
            case WM_XBUTTONDOWN: keyCode = HIWORD(mouse->mouseData) == XBUTTON1 ? VK_XBUTTON1 : VK_XBUTTON2;
                down = true;
                break;
            case WM_LBUTTONUP: keyCode = VK_LBUTTON;
                up = true;
                break;
            case WM_RBUTTONUP: keyCode = VK_RBUTTON;
                up = true;
                break;
            case WM_MBUTTONUP: keyCode = VK_MBUTTON;
                up = true;
                break;
            case WM_XBUTTONUP: keyCode = HIWORD(mouse->mouseData) == XBUTTON1 ? VK_XBUTTON1 : VK_XBUTTON2;
                up = true;
                break;
            case WM_MOUSEWHEEL: keyCode = HIWORD(mouse->mouseData) & 0x8000 ? 0x1000 : 0x1001;
                down = true;
                break;
            case WM_MOUSEHWHEEL: keyCode = HIWORD(mouse->mouseData) & 0x8000 ? 0x1002 : 0x1003;
                down = true;
                break;
            }
            if (keyCode)
