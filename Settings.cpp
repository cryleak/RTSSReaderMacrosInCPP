...
namespace {
	...
	std::wstring formatFileTime(const std::filesystem::path& path) {
		...
		wchar_t buffer[64]{};
		GetDateFormatEx(LOCALE_NAME_USER_DEFAULT, DATE_SHORTDATE, &local, nullptr, buffer, static_cast<int>(std::size(buffer)), nullptr);
		std::wstring result = buffer;
		result += L" ";
		GetTimeFormatEx(LOCALE_NAME_USER_DEFAULT, TIME_NOSECONDS, &local, nullptr, buffer, static_cast<int>(std::size(buffer)));
		result += buffer;
		return result;
	}
...
bool GtaProfileImporter::importProfile(const GtaProfile& profile, MacroSettings& settings,
	...
	Microsoft::WRL::ComPtr<IXMLDOMDocument2> document;
