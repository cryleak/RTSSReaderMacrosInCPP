...
namespace
{
    ...
    {
        std::wstring result;
        for (; value && *value; ++value) result += static_cast<unsigned char>(*value);
        return result;
    }
    ...
    bool beginGet(const std::wstring& url, HttpHandles& handles, DWORD& status, std::string& error, bool apiRequest)
    {
        ...
        handles.session = WinHttpOpen(userAgent.c_str(), WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, nullptr,
                                      nullptr, 0);
        if (!handles.session)
        {
        ...
        handles.request = WinHttpOpenRequest(handles.connection, L"GET", path.c_str(), nullptr, nullptr,
                                             nullptr, secure ? WINHTTP_FLAG_SECURE : 0);
        if (!handles.request)
        {
        ...
        if (!WinHttpSendRequest(handles.request, headers, static_cast<DWORD>(-1), nullptr, 0, 0, 0) ||
