...
std::wstring localizedKeyName(WORD vkCode) {
  ...
  wchar_t name[64]{};
  int length = GetKeyNameTextW(keyNameParam, name, static_cast<int>(std::size(name)));
...
std::string keyName(WORD vkCode) {
  ...
  if (scanCode && GetKeyNameTextA(static_cast<LONG>(scanCode) << 16, name, static_cast<int>(sizeof(name)))) {
    return lower(name);
  }
